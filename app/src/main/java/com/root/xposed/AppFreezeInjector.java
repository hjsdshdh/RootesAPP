...
public class AppFreezeInjector {
    ...
    private boolean unfreeze(Context context, String packageName) {
        ...
        try {
            Uri uri = Uri.parse("content://com.root.system.SceneFreezeProvider");
        ...
        try {
            ...
            if ((Boolean) (method.invoke(packageManager, packageName))) {
                ...
                if (unfreeze(context, packageName)) {
                    // -> 通过Scene解冻就完事了咯
                    Toast.makeText(context, "已通过玩机百宝箱解冻应用：" + packageName, Toast.LENGTH_SHORT).show();
                }

                // 方式1： 由Scene通过ROOT启动
                // intent.setClassName("com.root.system", "com.root.system.activities.ActivityQuickStart");
