...
class ChargeTimeView : View {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (point in 0..minutes) {
            if (point % 5 == 0) {
                if (point % columnSize == 0) {
                    ...
                    canvas.drawText(
                        text,
                        (point * ratioX).toFloat() + innerPadding,
                    ...
                    canvas.drawLine(
                        (point * ratioX).toFloat() + innerPadding, innerPadding,
                        (point * ratioX).toFloat() + innerPadding, this.height - innerPadding, paint
                    )
                } else {
                    paint.strokeWidth = 2f
                    paint.color = Color.parseColor("#aa888888")
            ...
            if (point % 10 == 0) {
                if (point > 0) {
                    canvas.drawText(
                        ...
                        innerPadding + ((maxY - point) * ratioY).toFloat() + textSize / 2.2f,
                        paint
                    )
                    canvas.drawCircle(
                        innerPadding,
                        innerPadding + ((maxY - point) * ratioY).toFloat(),
                        potintRadius,
                        paint
                    )
                }
        ...
        if (startTime != null) {
            ...
            val startX = ((first.startTime - startTime) / 60000f * ratioX) + innerPadding
            val endX = ((last.endTime - startTime) / 60000f * ratioX) + innerPadding
            pathFilterAlpha.moveTo(startX.toFloat(), stratY - (first.capacity * ratioY))
            for (sample in samples) {
                val currentX = ((sample.startTime - startTime) / 60000f * ratioX) + innerPadding
                pathFilterAlpha.lineTo(currentX.toFloat(), stratY - (sample.capacity * ratioY))
            }
            pathFilterAlpha.lineTo(endX.toFloat(), stratY - (last.capacity * ratioY))
        }
